  ...
  return data.map(
    ({
      ...
      return {
        ...
        address: address
          ? `${address.houseFlat}, ${address.street}, ${address.city}${
              address.district?.length ? ', ' + address.district : ''
            }${address.state?.length ? ', ' + address.state : ''}, ${getCodeValueFromCatalog(
              'country',
              address.countryIso,
            )}, ${address.postalCode}`
          : '',
        ...
        virology,
        treatmentReferral: `${treatmentReferral.doctor} / ${treatmentReferral.clinic}`,
