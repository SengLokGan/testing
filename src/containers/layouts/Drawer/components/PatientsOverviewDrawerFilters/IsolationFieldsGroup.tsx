...
const IsolationFieldsGroup = ({
  ...
  const onCheckboxClick = useCallback((event: React.ChangeEvent<HTMLInputElement>, name) => {
  ...
  return (
    <>
      <Typography variant="labelMCapsSB" sx={{ color: ({ palette }) => palette.text.secondary, mb: 1 }}>
        {t(`filter.isolation`)}
      </Typography>
      <FormGroup>
