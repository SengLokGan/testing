...
const AddDialysisMachineForm = () => {
  ...
  const defaultValues: DialysisMachineForm = {
    ...
    isolationGroupId: (isEditing && machine?.isolationGroup?.id) || nonInfectionIsolationGroupId,
    locationId: (isEditing && machine?.location?.id) || 0,
    maintenanceFrom:
  ...
  const updateAvailableLocationsList = async () => {
    ...
    try {
      ...
      let dataClone: Location[] = [...(data as any[])];
