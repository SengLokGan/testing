...
import { ServiceModalName, DrawerType, DoctorTypes, HdPrescriptionStatuses, DialyzerUseType, HdType } from '@enums';
...
const AddHdPrescriptionForm = () => {
  ...
  const defaultValues: HdPrescriptionForm = {
    ...
    dialyzerUseType: formData?.dialyzerUseType || DialyzerUseType.Single,
    dialyzerBrand: formData?.dialyzerBrand,
  ...
  useEffect(() => {
    if (formData?.schedule?.recurrent || formData?.schedule?.adHoc) {
      ...
      if (formData.schedule.adHoc) {
        HdSchedulingFormData = {
          hdType: formData.schedule.recurrent ? HdType.Recurrent : HdType.AdHoc,
          dateShifts: formData.schedule.adHoc.dateShifts.map(({ duration, shiftId, date }) => ({
            duration,
            shiftId,
            date: new Date(date),
          })),
  ...
  useDeletedUserValidation('prescribedBy', setError, prescribedByField as any);
    ...
    if (patientId && prescribedBy && schedulingFormData) {
      let hdPrescriptionPayload: HdPrescriptionRequest = {
        ...
        prescribedBy: {
          ...
          internalDoctorId: !isExternalDoctor ? prescribedBy.specialities[0].id : undefined,
        ...
        dialyzerBrand: (data.dialyzerBrand as any).value,
