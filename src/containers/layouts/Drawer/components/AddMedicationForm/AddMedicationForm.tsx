...
const AddMedicationForm = () => {
  ...
  const { id: patientId, type } = selectDrawerPayload(DrawerType.Medication);
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      day: formData?.place === MedicationPlaces.InCenter || !formData?.day ? '' : formData.day,
      meal: formData?.meal,
  ...
  const onSubmit = ({ doctorsName, ...data }: MedicationForm) => {
    if (patientId) {
      const medicationData = {
        medication: {
          place: data.place,
          medicationUid: data?.nameDrug?.uid ?? formData?.id,
