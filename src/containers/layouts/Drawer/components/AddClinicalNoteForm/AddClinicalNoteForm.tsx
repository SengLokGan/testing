...
const AddClinicalNoteForm = () => {
  ...
  const { id: patientId } = selectDrawerPayload(DrawerType.ClinicalNotesForm);
  ...
  const onSubmit = (data) => {
    if (patientId) {
      dispatch(addOrEditClinicalNote({ isAdding: !formData, clinicalNote: data, patientId: +patientId }));
    }
  };
