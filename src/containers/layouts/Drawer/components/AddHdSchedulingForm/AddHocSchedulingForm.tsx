...
import { selectPatientIsolationStatus } from '@store/slices/patientSlice';
import { format } from 'date-fns';
import { DateShifts } from '@containers/layouts/Drawer/components/AddHdSchedulingForm/DateShifts';
import { selectShiftDictionary } from '@store/slices/hdPrescriptionSlice';
...
export const AddHocSchedulingForm = ({ control, watch, trigger, isValid, setValue }: AddHocSchedulingFormProps) => {
    ...
    return API.post('/pm/prescriptions/ad-hoc/available', params)
      .then(({ data }: AxiosResponse<{ date: string; shifts: { id: string; name: string }[] }[]>) => {
        ...
        if (dateShifts.length) {
          dateShifts.forEach(({ date, shiftId }) => {
            if (availabilityOptions[date!]) {
              if (!availabilityOptions[date!].find(({ value }) => value === shiftId)) {
                availabilityOptions[date!].push({ value: shiftId, label: getShiftName(shiftId) });
              }
            } else {
              availabilityOptions[date!] = [{ value: shiftId, label: getShiftName(shiftId) }];
            }
          });
        }
  ...
  const getSelectedDateShifts = () => {
    return controlledFields
      ...
      .map((field) => ({ date: field.date ? format(field.date, 'yyyy-MM-dd') : null, shiftId: field.shiftId }));
  };

  const appendBlock = () => {
    getAvailability().then(() => {
      append({ date: null, shiftId: '', duration: 240 });
    });
  };
  ...
  return (
    <>
      ...
      <Button
        onClick={appendBlock}
        variant={'outlined'}
        disabled={controlledFields.length === 7 || !isValid || !Object.keys(availability).length}
