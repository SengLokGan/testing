...
import { selectUser, setTenant, setBranch } from '@store/slices/userSlice';
  ...
  return (
    ...
    >
      ...
      <Stack
        ...
        width={{ xs: 1, md: 'unset' }}
        sx={[
          isDrawerPlace && { display: { xs: 'flex', sm: 'none' } },
          {
            justifyContent: {
              xs: 'space-between',
              md: 'flex-start',
      ...
      >
        <Typography variant="paragraphXS">
          {`${t('appName')}.${mainVersion}${buildVersion ? `.${buildVersion}` : ''} ${hashVersion}`}
        </Typography>
        <Typography
          variant="paragraphXS"
          sx={{
