import { PerformLabOrderRequestParams, PerformLabTestForm, ServiceModalProps } from '@types';
...
const PerformLabTestModal = ({ index }: ServiceModalProps) => {
  ...
  const showForm = procedureOptions.length > 0 && nursesOptions.length > 0;
  ...
  const defaultValues: PerformLabTestForm = {
    ...
    comments: defaultFormValues.defaultFormValues ?? '',
  };

  const { handleSubmit, reset, control, watch, setValue } = useForm<PerformLabTestForm>({
    mode: 'onBlur',
    defaultValues,
  ...
  return (
    ...
    >
      ...
      <DialogContent dividers sx={{ p: 2 }}>
        ...
        {!showForm && (
          <Stack direction="column" data-testid="urgentLabTestSkeletonForm">
