import { Dialog, DialogContent, Grid, Skeleton } from '@mui/material';
...
interface VaccineMedicationAdministeringModalPayload {
  patientId: number;
  vaccine?: DialysisVaccinationResponse;
...
const VaccineMedicationAdministeringModal = ({ index }: ServiceModalProps) => {
  ...
  const { patientId, vaccine, medication, status, serviceType, mode }: VaccineMedicationAdministeringModalPayload =
    selectServiceModal(ServiceModalName.VaccineMedicationAdministeringModal);
  ...
  const openConfirmModal = () => {
    ...
    if (status === VaccinationMedicationAdministeringStatus.Administering) {
      isMedication
        ? (title = tMedication('form.youAreGoingToAdminister', medicationTranslatingData))
        : (title = tVaccination('form.youAreGoingToAdminister', vaccineTranslatingData));
