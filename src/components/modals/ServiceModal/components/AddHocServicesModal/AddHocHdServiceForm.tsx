...
import { AddHocEventFormType, AvailabilityShifts } from '@types';
...
import { NoticeBlockType, ServiceModalName } from '@enums/components';
...
export const AddHocHdServiceForm = ({
    ...
    if (patient) {
      try {
        ...
        const availability = data.reduce((acc, { date, shifts }) => {
          return {
            ...acc,
            [date]: shifts.map(({ id, name }) => ({ value: id, label: `${name} ${t('shift')}` })),
          };
        }, {});
  ...
  useEffect(() => {
    place === AddServiceModalPlace.SHIFT &&
      setShiftOptions(shifts.map((shift) => ({ label: `${shift.name} ${t('shift')}`, value: shift.id })));
  }, []);
