...
import type { AccessForm, PreDialysisRequest } from '@types';
...
import { StartHdModal } from './components/StartHdModal';
...
export const DialysisPreHdStep = () => {
  ...
  const setDialyzerGroupDefaultValues = () => ({
    useType: preHd?.dialyzer?.useType || DialyzerUseType.Reuse,
    dialyzerBrand: { label: preHd?.dialyzer?.brand || '', value: preHd?.dialyzer?.brand || '' },
    dialyzerSurfaceArea: typeof preHd?.dialyzer?.surfaceArea === 'number' ? preHd?.dialyzer?.surfaceArea : '',
    dialyzerPrimedBy: {
      label: preHd?.dialyzer?.primedBy?.name || userNurse?.name || '',
      value: preHd?.dialyzer?.primedBy?.id || userNurse?.id || '',
    },
    dialyzerReuseNum: preHd?.dialyzer?.reuseNumber || '',
    sterilantVe: !!preHd?.dialyzer?.beforeSterilant?.test,
    residualVe: !!preHd?.dialyzer?.afterSterilant?.test,
    dialyzerSterilantVeComment: preHd?.dialyzer?.comment || '',
    dialyzerTestedBy: {
      label: preHd?.dialyzer?.beforeSterilant?.testedBy?.name || userNurse?.name || '',
      value: preHd?.dialyzer?.beforeSterilant?.testedBy?.id || userNurse?.id || '',
    },
    residualTestedBy: {
      label: preHd?.dialyzer?.afterSterilant?.testedBy?.name || userNurse?.name || '',
      value: preHd?.dialyzer?.afterSterilant?.testedBy?.id || userNurse?.id || '',
    },
  });
  ...
  const defaultValues: PreHDForm = {
    ...
    notes: preHd?.notes || '',
    newDialyzer: preHd?.dialyzer?.primedBy
      ? !(!!preHd?.dialyzer?.beforeSterilant?.test && !!preHd?.dialyzer?.afterSterilant?.test)
      : false,
  };
    ...
    handleSubmit((data) => {
      ...
      const preparedData: PreDialysisRequest = {
        ...
        dialyzer: {
          ...
          dialyzer: data.dialyzerBrand?.value,
          brand: data.dialyzerBrand?.value,
          surfaceArea: data.dialyzerSurfaceArea,
          primedBy: data.dialyzerPrimedBy.value,
          reuseNumber: data?.dialyzerReuseNum,
          beforeSterilant:
            !data.newDialyzer && data.useType === DialyzerUseType.Reuse
              ? {
  ...
  return (
    <>
      {(isLoading || (!preHd && !nursesOptions.length)) && (
      ...
      >
        <DialysisPreHdStepInitialInfoGroup control={control} watch={watch} trigger={trigger} />
        <DialysisPreHdWeightAndUfGroup control={control} watch={watch} setValue={setValue} trigger={trigger} />
        ...
        <DialysisPreHdStepAnticoagulantGroup control={control} />
        ...
        />
        <DialysisPreHdStepDialysateGroup control={control} />
        <DialysisPreHdNote control={control} />
        {!isDisabledInterface && (
