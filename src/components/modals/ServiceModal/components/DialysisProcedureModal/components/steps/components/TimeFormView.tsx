...
import { validatorFutureTime, validatorRequired, validatorTimeNotEarlierThan } from '@validators';
...
export const TimeFormView = <T extends FieldValues>({
  ...
  if (type === TimeFormType.FINISH && startTime) {
    timeFieldValidation.notEarlierThan = validatorTimeNotEarlierThan(new Date(startTime));
  }

  return (
    <Dialog open={open} onClose={onClose} data-testid={`${type === TimeFormType.START ? 'start' : 'finish'}HdModal`}>
      <Box sx={{ m: 0, p: 2 }}>
      ...
      <DialogContent dividers sx={(theme) => ({ width: theme.spacing(48) })}>
