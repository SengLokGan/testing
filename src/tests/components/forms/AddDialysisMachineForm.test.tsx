...
import { dialysisMachineIsolationGroupFixture, dialysisMachineFullFixture } from '@src/tests/fixtures';
...
describe('AddDialysisMachineForm', () => {
  ...
  it('should show "confirm modal", when user tried to cancel form insertions', async () => {
    await user.type(screen.getByTestId('nameTextInput'), 'test');
    await user.click(screen.getByTestId('cancelDialysisMachineCreationButton'));
