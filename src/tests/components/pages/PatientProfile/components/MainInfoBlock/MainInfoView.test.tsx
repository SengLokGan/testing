...
describe('View of the patient documents', () => {
  ...
  it('should have image if patient has photo link', async () => {
    const { getByAltText } = render(<MainInfoView patient={patient} />);
  ...
  it('avatar should have first letter of the name if patient has no photo link', async () => {
    patient.photoPath = '';
    const { getByText } = render(<MainInfoView patient={patient} />);
    const firstLetterOfName = getByText('C');
    expect(firstLetterOfName.textContent).toContainEqual('C');
  });

  it('should show open patient photo modal on avatar click', async () => {
    patient.photoPath = '#';
    const { getByAltText, getByText } = render(<MainInfoView patient={patient} />);
    const avatar = getByAltText('avatar');
    await user.click(avatar);
    const patientPhotoModalTitle = getByText('profile.patientPhoto');
