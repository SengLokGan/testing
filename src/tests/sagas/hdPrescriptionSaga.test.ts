...
import {
  addHdPrescriptionSaga,
  deleteHdPrescriptionSaga,
  getShiftDictionarySaga,
  viewHdPrescriptionsSaga,
} from '@sagas/hdPrescriptionSaga';
import {
  addHdPrescription,
...
describe('HD Prescription Sagas', () => {
  ...
  describe('getShiftDictionarySaga', () => {
    it('should correctly fetch shifts', async () => {
      API.get = jest.fn().mockResolvedValue({ data: {} });
      const dispatched: Action[] = [];

      await runSagaHandler(dispatched, {}, getShiftDictionarySaga, {
        type: getShiftDictionary.type,
      });

      expect(API.get).toHaveBeenCalledWith('/pm/shifts');
      expect(dispatched).toEqual([
        {
    ...
    it('should correctly handle error', async () => {
      ...
      expect(dispatched).toEqual([{ type: addHdPrescriptionError.type, payload: 'Error' }]);
    });
  });
});
