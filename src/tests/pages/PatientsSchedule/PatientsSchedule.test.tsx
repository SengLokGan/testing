...
import { isolationsGroup, shifts, appointments, dayTasks } from '@src/tests/fixtures/patientsSchedule';
...
import { AppointmentSchedule } from '@types';
...
describe('PatientsSchedule', () => {
  ...
  it('should render schedule header', () => {
    ...
    expect(screen.getByText(`${shifts[0].name} shift`)).toBeInTheDocument();
    expect(screen.getByText('12:00 PM')).toBeInTheDocument();
    expect(screen.getByText(`${shifts[1].name} shift`)).toBeInTheDocument();
    expect(screen.getByText('17:00 PM')).toBeInTheDocument();
    expect(screen.getByText(`${shifts[2].name} shift`)).toBeInTheDocument();
  ...
  it('should render schedule table', () => {
    ...
    expect(screen.getByTestId('scheduleTable')).toBeInTheDocument();
    expect(screen.getByTestId('dayTasksRow')).toBeInTheDocument();
  ...
  it('should render schedule dayTasks', () => {
    ...
    expect(screen.getByTestId('dayTasksRow')).toBeInTheDocument();
    expect(screen.getByText('dayTasks')).toBeInTheDocument();
    expect(screen.getAllByTestId('dayTask')).toHaveLength(dayTasks.length);
    expect(screen.getByText('events.QUARTERLY_BLOOD_TEST')).toBeInTheDocument();
    expect(screen.getByText(dayTasks[1].patientName!)).toBeInTheDocument();
    expect(screen.getByText(dayTasks[2].patientName!)).toBeInTheDocument();
  });
  ...
  it('should render schedule dayTask correct position', () => {
    ...
    expect(screen.getByTestId('dayTask')).toHaveStyle('left: 0%; width: 33.333333333333336%;');
  });
