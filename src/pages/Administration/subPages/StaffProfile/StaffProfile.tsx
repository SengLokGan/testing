...
export const StaffProfile = () => {
  ...
  return (
    <>
      ...
      <Stack direction="column" sx={{ width: 1, height: 1, bgcolor: (theme) => theme.palette.surface.default }}>
                  ...
                  staffUser?.files?.length > 0 ? (
                    <Stack direction="column" spacing={1} sx={{ width: 1, maxWidth: ({ spacing }) => spacing(31) }}>
                      {staffUser.files?.map((file: FileDocument) => (
                        <FileLink withIcon={false} patientId={staffUser.id} key={file.id} file={file} />
                      ))}
                    </Stack>
                  ) : undefined
                }
