...
import { LabOrderType, ServiceModalName, SnackType } from '@enums/components';
import { AppointmentEventPlace } from '@enums/global';
import { getServicesSaga } from '../dialysisSaga';
import i18n from 'i18next';
import { AddHocEventFormType, AddHocEventTypes } from '@types';
...
function* getDayEvents() {
  ...
  } = yield call(API.get, `pm/calendar/daily-tasks?date=${dateToServerFormat(date)}`);

  const labTestEvents = appointments.map((appointment) => ({
    ...appointment,
    type: AddHocEventTypes.LAB_TEST,
    isAllDay: true,
  }));
  return [...events, ...labTestEvents];
}
