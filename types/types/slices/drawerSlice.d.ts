...
export type WithDrawerType<T> = T & {
  type: DrawerType;
};
export type WithDrawerStatuses<T> = T & {
  statuses: DrawerStatuses;
};
export type DrawerWithOptionalStatuses = Drawer &
  Partial<{
    statuses: Partial<DrawerStatuses>;
  }>;
export type DrawerWithTransitionStatus<T> = T & {
